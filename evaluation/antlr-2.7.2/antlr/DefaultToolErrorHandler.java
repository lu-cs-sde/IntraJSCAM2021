 ...
 *
 * $Id:
 * //depot/code/org.antlr/release/antlr-2.7.2/antlr/DefaultToolErrorHandler.java#1
 * $
 */
...
class DefaultToolErrorHandler implements ToolErrorHandler {
  DefaultToolErrorHandler(antlr.Tool tool) { antlrTool = tool; }
   ...
   */
  private void dumpSets(String[] output, int outputStartIndex, Grammar grammar,
                        boolean lexicalAnalysis, int depth, Lookahead[] sets) {
    ...
    for (int i = 1; i <= depth; i++) {
      ...
      if (lexicalAnalysis) {
        ...
        line.append(
            sets[i].fset.toString(",", grammar.tokenManager.getVocabulary()));
      }
  ...
  public void warnAltAmbiguity(Grammar grammar, AlternativeBlock blk,
                               boolean lexicalAnalysis, int depth,
                               Lookahead[] sets, int altIdx1, int altIdx2) {
    StringBuffer line = new StringBuffer(100);
    ...
    antlrTool.warning(output, grammar.getFilename(), blk.getLine(),
                      blk.getColumn());
  }
