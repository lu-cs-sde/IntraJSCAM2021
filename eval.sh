...
zsh run_eval.sh antlr-2.7.2/antlr $N_ITER $N_ITER_SS "ANTLR" $CLASSPATH_ANTLR


# ### EXCEUTES ALL THE EXPERIMENTS ON FOP
zsh run_eval.sh fop-0.95/src $N_ITER $N_ITER_SS "FOP" $CLASSPATH_FOP


# # ### EXCEUTES ALL THE EXPERIMENTS ON PMD
zsh run_eval.sh pmd-4.2.5/src $N_ITER $N_ITER_SS "PMD" $CLASSPATH_PMD


# ### EXCEUTES ALL THE EXPERIMENTS ON JFREE
zsh run_eval.sh jfreechart-1.0.0/source $N_ITER $N_ITER_SS "JFreeChart" $CLASSPATH_JFREECHART
